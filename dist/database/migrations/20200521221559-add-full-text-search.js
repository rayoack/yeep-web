"use strict";const vectorName = '_search';
    ...
    queryInterface.sequelize.transaction((t) =>
      Promise.all(Object.keys(searchObjects).map((table) =>
        ...
        `, { transaction: t })
          .then(() =>
            queryInterface.sequelize.query(`
                UPDATE ${table} SET ${vectorName} = to_tsvector('english', ${searchObjects[table].join(" || ' ' || ")}) || to_tsvector('portuguese', ${searchObjects[table].join(" || ' ' || ")})
              `, { transaction: t })
          ).then(() =>
            queryInterface.sequelize.query(`
